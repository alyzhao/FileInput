$(document).ready(function() {
     ...
     */
    function uploadImg(formdata, url, directory, imgPrefix,callback) {
        formdata.append("directory", directory);
        formdata.append("imgPrefix", imgPrefix);
        $.ajax({
            url: url,
            type: "post",
            data: formdata,
    ...
    jQuery.fn.extend({
        ...
        /* 多图上传时图片的移动
         * @param direction 移动的方向 "left"向左， "right"向右
         * @return 移动成功返回true,否则返回false
         */
        multipleMove: function (direction) {
            var parentCell = this.parents(".multiple-cell");
            var index = parentCell.index(".multiple-cell");
            if (direction == "left") {
                if(index == 0) {
                    layer.msg("已经在最左边了！", {icon: 2, time: 2000})
                    return false;
                }
                parentCell.prev(".multiple-cell").before(parentCell.remove());
            } else if (direction == "right") {
                if(index == parentCell.siblings(".multiple-cell").length) {
                    layer.msg("已经在最右边了！", {icon:2, time: 2000});
                    return false;
                }
                parentCell.next(".multiple-cell").after(parentCell.remove());
            }
            return true;
        }
    });
    ...
    $(".file-upload-button").on("click", function() {
        ...
        if (upfileWrap.find("input[multiple=nomultiple]").length > 0) {
            ...
            uploadImg(singleFormData, "/UploadImg/UploadImg/", "/Content/images/singleImgs/", "single", function(data) {
                // 单图上传成功的回调函数
                if (data.error == 0) {
                    upfileWrap.find(".data-imgurl").html(data.imgUrlList[0].imgUrl);
            ...
            uploadImg(multFormData, "/UploadImg/UploadImg/", "/Content/images/multipleImgs/", "multiple", function(data) {
                // 多图上传回调函数
                if (data.error == 0) {
                    ...
                    for (var i = 0; i < length; i++) {
                        ...
                        $("#" + item.imgName).children(".multimgurl").html(item.imgUrl);
                    }
                    layerMsg({type: "success", words: "上传成功！"});
                    multFormData = new FormData();
                } else {
    ...
    $(".input-upload[multiple=multiple]").on("change", function() {
        ...
        for (var i = 0; i < length_file; i++) {
            ...
            if (!fileItem.type.match(regImg)) {
                layerMsg({words: "只能上传jpg、jpeg、png、gif格式的图片！", type: "warning"});
                return false;
            }
            // 向multFormData中添加数据
            !multFormData && (multFormData = new FormData());
            multFormData.append("multpic" + ++nowMaxNum, fileItem)
            ...
            reader.onload = function(e) {
                ...
                var DOMElement = `<div class="multiple-cell" id="multpic${++maxNum}">
                                    ...
                                    <div class="opera">
                                        <div class="shift">
                                            <button class="btn btn-default scroll-prev"><i class="fa fa-arrow-left"></i></button>
